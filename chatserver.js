...
var httpsOptions = {
  key: fs.readFileSync('/etc/letsencrypt/live/bright.ikirize.net/privkey.pem'),
  cert: fs.readFileSync('/etc/letsencrypt/live/bright.ikirize.net/cert.pem')
};
...
var httpsServer = https.createServer(httpsOptions, function(request, response) {
  ...
  if(request.url.indexOf("/?") > -1){
    ...
    var token=url_parts.query['sekai'];
    if(token){
      token=JSON.parse(token)
      
      var hash = crypto.createHmac('sha1', 'sekai').update(token['name']+token['position']).digest('hex')
      console.log(token['hash'])
      console.log(hash);
      if (token['hash'] == hash){
      fileServer.serveFile("index.html",200,{'Set-cookie': ' token='+token['name']+":"+token['positions']+";"},request, response);
      }else{
